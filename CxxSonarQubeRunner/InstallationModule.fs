...
let GetVeraPath() =
    if isWindowSystem then
        ...
        if not(UnixDist.Contains("Ubuntu")) then
            HelpersMethods.cprintf(ConsoleColor.Red, sprintf "Vera++ not aviable in non Ubuntu: %A" System.Runtime.InteropServices.RuntimeInformation.OSDescription)
            ""
        else
            try
                ...
                if ret = 0 then
                    HelpersMethods.cprintf(ConsoleColor.Green, sprintf "Vera is available: %s" "vera++")
                    "vera++"
                 else
                    raise (new Exception(sprintf "Vera++ not installed in Machine, install Vera with 'brew install vera++' or 'apt install vera++': return code %i" ret))
                    ""

            with
            | ex -> raise (new Exception(sprintf "Vera++ not installed in Machine, install Vera with 'brew install vera++' or 'apt install vera++': error message %s" ex.Message))
                    ""
...
let GetRatsPath() =
    if isWindowSystem then
        ...
        if isLinuxSystem  then
            HelpersMethods.cprintf(ConsoleColor.Red, sprintf "Rats not aviable in Unix: %A" System.Runtime.InteropServices.RuntimeInformation.OSDescription)
            ""
        else
            try
                ...
                if ret = 0 then
                    ...
                    raise (new Exception(sprintf "rats not installed in Machine, install Vera with 'brew install rats': return code %i" ret))
                    ""

            with
            | ex -> raise (new Exception(sprintf "rats not installed in Machine, install rats with 'brew install rats': error message %s" ex.Message))
                    ""
